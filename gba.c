...
void waitForVBlank(void) {
    ...
    // (This prevents counting one VBlank more than once if app is too fast)
...
void setPixel(int x, int y, u16 color) {
    volatile unsigned short *addy = (videoBuffer + (y * WIDTH) + x);
    *addy = color;
}
...
void drawRectDMA(int x, int y, int width, int height, volatile u16 color) {
    for (int i = y; i < height + y; i++) {
        DMA[3].src = &color;
        DMA[3].dst = (videoBuffer + (i * WIDTH) + x);
